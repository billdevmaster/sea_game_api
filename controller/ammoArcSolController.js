...
const saveScore = async (req, res, connection) => {
	...
	let sql = "";
	if (hashServer == hash) {
		sql = `SELECT * FROM ammo_arcade_sol_score WHERE address='${address}'`;
		try {
	      	connection.query(sql, (err1, result) => {
		        ...
		        if (result.length > 0) {
		        	...
		        	if (score * 1 > result[0].score) {
		        		sql = `UPDATE ammo_arcade_sol_score SET score=${score} WHERE address='${address}'`;
		        	...
		        	// insert
		        	sql = `INSERT INTO ammo_arcade_sol_score (
			            address, 
			            score) VALUES (
			            '${address}', 
			            ${score * 1})`;
...
const getLeaderboard = async (req, res, connection) => {
	...
	let sql = "";
	sql = `SELECT * FROM ammo_arcade_sol_score ORDER BY score DESC LIMIT ${10 * (page * 1 - 1)}, 10`;
	try {
		connection.query(sql, (err1, result) => {
		    if (err1) throw err1;
		    console.log(result);
